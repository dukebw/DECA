...
class DECA(object):
    ...
    def _setup_renderer(self, model_cfg):
        ...
        self.uv_face_eye_mask = F.interpolate(
            mask, [model_cfg.uv_size, model_cfg.uv_size]
        ).to(self.device)
        ...
        self.mean_texture = F.interpolate(
            mean_texture, [model_cfg.uv_size, model_cfg.uv_size]
        ).to(self.device)
    ...
    def decode(self, codedict):
        ...
        detail_normal_images = (
            F.grid_sample(uv_detail_normals, ops["grid"], align_corners=False)
            * ops["alpha_images"]
        )
        ...
        visdict = {
            ...
            "landmarks2d": util.tensor_vis_landmarks(
                images, landmarks2d, isScale=False
            ),
            "landmarks3d": util.tensor_vis_landmarks(
                images, landmarks3d, isScale=False
            ),
