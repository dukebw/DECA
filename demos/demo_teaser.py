...
def main(args):
    ...
    for i in range(len(testdata)):
        ...
        max_yaw = 30
        yaw_list = (
            list(range(0, max_yaw, 5))
            + list(range(max_yaw, -max_yaw, -5))
            + list(range(-max_yaw, 0, 5))
        )
        for k in yaw_list:  # jaw angle from -50 to 50
            ...
            euler_pose[
                :, 0
            ] = 0  # (torch.rand((self.batch_size))*60 - 30)*(2./euler_pose[:,1].abs())
            euler_pose[
                :, 2
            ] = 0  # (torch.rand((self.batch_size))*60 - 30)*(2./euler_pose[:,1].abs())
    ...
    for i in range(len(yaw_list)):
        ...
        grid_image_all = rescale(
            grid_image_all, 0.6, multichannel=True
        )  # resize for showing in github
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s",
        "--savefolder",
        default="TestSamples/teaser/results",
        type=str,
