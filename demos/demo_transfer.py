...
def main(args):
    ...
    testdata = datasets.TestData(
        args.image_path, iscrop=args.iscrop, face_detector=args.detector
    )
    expdata = datasets.TestData(
        args.exp_path, iscrop=args.iscrop, face_detector=args.detector
    )
    ...
    cv2.imwrite(
        os.path.join(savefolder, name + "_animation.jpg"), deca.visualize(id_visdict)
    )
    ...
    for save_type in ["reconstruction", "animation"]:
        ...
        if args.saveDepth:
            ...
            cv2.imwrite(
                os.path.join(savefolder, name, save_type, name + "_depth.jpg"),
                util.tensor2image(depth_image[0]),
            )
        if args.saveKpt:
            np.savetxt(
                os.path.join(savefolder, name, save_type, name + "_kpt2d.txt"),
                opdict["landmarks2d"][0].cpu().numpy(),
            )
            np.savetxt(
                os.path.join(savefolder, name, save_type, name + "_kpt3d.txt"),
                opdict["landmarks3d"][0].cpu().numpy(),
            )
        if args.saveObj:
            deca.save_obj(
                os.path.join(savefolder, name, save_type, name + ".obj"), opdict
            )
        if args.saveMat:
        ...
        if args.saveImages:
            for vis_name in [
                ...
                cv2.imwrite(
                    os.path.join(
                        savefolder, name, save_type, name + "_" + vis_name + ".jpg"
                    ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i",
        "--image_path",
        default="TestSamples/examples/IMG_0392_inputs.jpg",
        type=str,
    ...
    parser.add_argument(
        "-s",
        "--savefolder",
        default="TestSamples/animation_results",
        type=str,
