...
def main(args):
    ...
    for i in tqdm(range(len(testdata))):
        ...
        if args.saveDepth:
            ...
            cv2.imwrite(
                os.path.join(savefolder, name, name + "_depth.jpg"),
                util.tensor2image(depth_image[0]),
            )
        if args.saveKpt:
            np.savetxt(
                os.path.join(savefolder, name, name + "_kpt2d.txt"),
                opdict["landmarks2d"][0].cpu().numpy(),
            )
            np.savetxt(
                os.path.join(savefolder, name, name + "_kpt3d.txt"),
        ...
        if args.saveVis:
            cv2.imwrite(
                os.path.join(savefolder, name + "_vis.jpg"), deca.visualize(visdict)
            )
        if args.saveImages:
            for vis_name in [
                "inputs",
                ...
                cv2.imwrite(
                    os.path.join(savefolder, name, name + "_" + vis_name + ".jpg"),
